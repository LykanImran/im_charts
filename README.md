...
- **📱 Mobile-Optimized Touch Ergonomics & Responsive Layout (`ChartLayoutMode`)**:
  ...
  - **Physics-Based Kinetic Inertia Scrolling**: Horizontal flings glide smoothly across historical candles using Flutter's native `FrictionSimulation`, decelerating naturally like native iOS/Android lists or TradingView.
  - **Floating "Jump to Real-Time" Action Pill**: Sleek glassmorphic button with pulsing live indicator dot (`❯ Real-time`) automatically appears when scrolled into history (`scrollOffset > 50px`) and animates smoothly back to real-time on tap with haptic feedback.
  - **Dedicated Mobile Long-Press Inspection Mode**: Clean 1-finger panning without crosshair interference. Long-pressing activates candle inspection with medium haptic confirmation, snapping across candles with subtle mechanical dial ticks (`HapticFeedback.selectionClick()`), and rendering an elevated OHLCV card above the user's thumb.
  - **Ultra-Smooth, Zero-Cling Finger Dragging**: Replaces frame-by-frame price rounding with continuous floating-point drag accumulation. Finger dragging of Buy/Sell orders, TP/SL brackets, and price alerts is 100% linear and fluid at 60/120Hz with tactile selection feedback.
  ...
  - **Responsive Drawing Toolbar Auto-Collapse**: Automatically collapses on mobile screens (`< 600px`) to an 18px strip to preserve valuable candle viewing real estate, expanding smoothly on tap.
