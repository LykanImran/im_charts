...
void main() async {
  ...
  if (!exampleDir.existsSync()) {
    stderr.writeln('❌ Error: Could not find "example" directory at ${exampleDir.path}');
    exit(1);
  }

  // 1. Get git remote URL
  stdout.writeln('📦 [1/4] Detecting Git remote origin URL...');
  final remoteResult = await Process.run('git', ['remote', 'get-url', 'origin']);
  if (remoteResult.exitCode != 0) {
    stderr.writeln('❌ Error: Failed to get Git remote origin: ${remoteResult.stderr}');
    exit(1);
  }
  ...
  Future<void> runCmd(String exe, List<String> args, {String? cwd}) async {
    final res = await Process.run(exe, args, workingDirectory: cwd ?? buildWebDir.path);
    if (res.exitCode != 0) {
  ...
  await runCmd('git', ['commit', '-m', 'Deploy example web showcase to GitHub Pages [skip ci]']);
