...
void main() {
  group('Technical Indicators Tests', () {
    ...
    test('VWAP calculates volume-weighted typical price and resets on session boundary', () {
      final startTime = DateTime(2026, 9, 14, 9, 15);
      final candles = <Candle>[
        // Day 1
        Candle(timestamp: startTime, open: 100, high: 105, low: 95, close: 100, volume: 1000),
        Candle(timestamp: startTime.add(const Duration(hours: 1)), open: 100, high: 110, low: 98, close: 104, volume: 2000),
        // Day 2 (session reset)
        Candle(timestamp: startTime.add(const Duration(days: 1)), open: 120, high: 125, low: 115, close: 120, volume: 500),
      ];
      ...
      expect(upperSeries.values[1]!, greaterThanOrEqualTo(vwapSeries.values[1]!));
      expect(vwapSeries.values[1]!, greaterThanOrEqualTo(lowerSeries.values[1]!));
    });

    test('MACD computes Fast/Slow EMA difference, Signal Line, and Histogram', () {
      final candles = generateMockCandles(45, 100.0);
      final macd = MACDIndicator(fastPeriod: 12, slowPeriod: 26, signalPeriod: 9);
