...
void main() {
  group('CandleBuilder Tests', () {
    test('Aggregates ticks into single candle within same timeframe interval', () {
      ...
      builder.onTick(Tick(timestamp: baseTime.add(const Duration(seconds: 15)), price: 105.0, volume: 5));
      builder.onTick(Tick(timestamp: baseTime.add(const Duration(seconds: 30)), price: 98.0, volume: 8));
      final lastCandle = builder.onTick(Tick(timestamp: baseTime.add(const Duration(seconds: 45)), price: 102.0, volume: 12));
    ...
    test('Creates new candle on timeframe interval boundary cross', () {
      ...
      expect(builder.candles[1].timestamp, equals(DateTime(2026, 9, 14, 9, 16, 0)));
    });
  });
}
